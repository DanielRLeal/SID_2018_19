...
public class MongoWrite implements MqttCallback{
	...
	//conteudo a escrever no servidor Paho, utilizado quando se for publicar mensagens
	//String content      = "teste123";
	//QoS 2 pois queremos rapidade de serviço mas que a mensagem apenas seja entregue uma vez
	//preferimos velocidade pois o que nos interessa é a variação dos valores ao longo do tempo
	int qos             = 0;
	...
	public MongoWrite(){
		...
		try {
			...
			//Cria o objecto cliente com os parametros do link de acesso e com o id de cliente
			//A MemoryPersistence permite que caso a ligação seja perdida este guarda as mensagens de QoS 1 e 2
			//em fila associadas ao ClientID para quando existir reconecção se receba as mensagens de volta
			...
			//ou seja é uma ligação nova que vai limpar as mensagens da ligação antiga que ficaram em fila
			//colocar a false para permitir receber mensagens que não foram entregues na sessão anterior
            ...
            //inicia objecto da mensagem a publicar
            MqttMessage message = new MqttMessage(content.getBytes());
            //Define a Qualidade de mensagem a utilizar
            message.setQos(qos);
            //publica a mensagem
	...
	public void messageArrived(String topicArrived, MqttMessage message) throws Exception {
		...
		//no caso do topico não for igual ao da mensagem que estamos a espera sai da função
		...
		System.out.println("data:" + dataValor + "; Temp:" + tempValor + "; Luz:" + lumValor + ";");
