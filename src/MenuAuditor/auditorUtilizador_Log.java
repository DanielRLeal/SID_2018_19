 ...
 *
 */
public class auditorUtilizador_Log extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					auditorUtilizador_Log window1 = new auditorUtilizador_Log(bd, username);
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	public auditorUtilizador_Log(BancoDeDados bd, String username) {
		this.username = username;
		this.bd = bd;
	}

	public auditorUtilizador_Log() {
		initialize();
	}
	...
	private void initialize() {
		...
		JLabel lblInicieASesso = new JLabel("Consulta de Logs dos Utilizadores");
		lblInicieASesso.setFont(new Font("Tahoma", Font.PLAIN, 12));
		lblInicieASesso.setBounds(159, 157, 196, 16);
		...
		JLabel lblMenu = new JLabel("Log Utilizador");
		panel.add(lblMenu);
